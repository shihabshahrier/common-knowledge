...
1. Create the `$CK_HOME/` directory and all subdirectories per the Store Layout spec.
2. Initialize git:
   ```bash
   git -C "$CK_HOME" init
   git -C "$CK_HOME" branch -M main   # or: git checkout -b main
   ```
3. Create `$CK_HOME/.gitignore`:
   ...
   *.swp
   ```
4. Create `$CK_HOME/README.md` with header + "No projects yet." (regenerated by `status`).
5. Create `$CK_HOME/_global/agent-config.md`, `tech-decisions.md`, `integrations.md` with section headers only.
6. Initial commit:
   ```bash
   git -C "$CK_HOME" add -A
   git -C "$CK_HOME" commit -m "chore: initialize common-knowledge store"
   ```
7. Go to Phase 8 — Report.
...
1. Run:
   ```bash
   grep -rn --include="*.md" --include="*.json" --include="*.sql" --include="*.yaml" \
     "$QUERY" "$CK_HOME" 2>/dev/null
   ```
2. Group results by project directory (parse path prefix).
3. Output: matched file + line number + 2 lines of context.
